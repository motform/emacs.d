...
** anzu
   If possible, I would like to replace anzu with builtin functionality. For another day.
   ...
   (use-package anzu
     ...
     (global-set-key (kbd "M-s-r") 'anzu-query-replace-at-cursor-thing)
...
*** Haskell
    Intero is THE BEST. Cannot fathom a world without it.
...
** avy
   Aka vim-sneak, but honestly so much better. The keys are mapped to a colemak-home-row.
...
** Ace-window
   I used to roll with ace-window and hydra, but they honestly did not offer anything over the combination of default keybinds and avy. On hold for now.
...
** Buffer Management
   Not used any more, probably yanked off stack overflow.
